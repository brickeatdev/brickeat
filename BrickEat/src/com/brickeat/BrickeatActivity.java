 ...
 */
public class BrickeatActivity extends SimpleBaseGameActivity {
	// ===========================================================
	// Constants
	// ===========================================================
	...
	private static final int ROWS = (int)Math.ceil((float)BrickeatActivity.COUNT / BrickeatActivity.COLUMNS);
	...
	public EngineOptions onCreateEngineOptions() {
		final Camera camera = new Camera(0, 0, BrickeatActivity.CAMERA_WIDTH, BrickeatActivity.CAMERA_HEIGHT);

		return new EngineOptions(true, ScreenOrientation.LANDSCAPE_FIXED, new RatioResolutionPolicy(BrickeatActivity.CAMERA_WIDTH, BrickeatActivity.CAMERA_HEIGHT), camera);
	}
	...
	public void onCreateResources() {
		...
		this.mSVGTestTextureRegions = new BaseTextureRegion[BrickeatActivity.COUNT];
		int i = 0;
	...
	public Scene onCreateScene() {
		...
		for(int i = 0; i < BrickeatActivity.COUNT; i++) {
			final int row = i / BrickeatActivity.COLUMNS;
			final int column = i % BrickeatActivity.COLUMNS;

			final float centerX = CAMERA_WIDTH / (BrickeatActivity.COLUMNS + 1) * (column + 1);
			final float centerY = CAMERA_HEIGHT / (BrickeatActivity.ROWS + 1) * (row + 1);

			final float x = centerX - BrickeatActivity.SIZE * 0.5f;
			final float y = centerY - BrickeatActivity.SIZE * 0.5f;
			...
			if(textureRegion instanceof TiledTextureRegion) {
				...
				final AnimatedSprite animatedSprite = new AnimatedSprite(x, y, BrickeatActivity.SIZE, BrickeatActivity.SIZE, tiledTextureRegion, vertexBufferObjectManager);
				animatedSprite.animate(500);
				scene.attachChild(animatedSprite);
			} else { 
				scene.attachChild(new Sprite(x, y, BrickeatActivity.SIZE, BrickeatActivity.SIZE, textureRegion, vertexBufferObjectManager));
			}
		}
